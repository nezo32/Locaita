...
class DQNAgent():
    def __init__(self, batch_size: int, max_memory_size: int, \
        ...
        self.dqn = DQN(screen_input_shape, action_input_shape, control_shape, learning_rate)
        self.dqn_target = DQN(screen_input_shape, action_input_shape, control_shape, learning_rate)
    ...
    def select_action(self, state, controls_state):
        action = self.dqn.model([tf.expand_dims(state, axis=0), tf.expand_dims(controls_state, axis=0)])
        action = tf.math.argmax(action, axis=1)
        return tf.cast(action, dtype=tf.int32)
    ...
    def load(self):
        self.dqn.load()
        self.dqn_target.optimizer.set_weights(self.dqn.optimizer.get_weights())
        self.dqn_target = tf.keras.models.clone_model(self.dqn.model)
    ...
    def learn(self):
        ...
        with tf.GradientTape() as tape:
            ...
            state_action_values = tf.stack([q[i, a1[i, 0]] for i in range(self.batch_size)])
            ...
            self.dqn.optimizer.apply_gradients(zip(grads, self.dqn.model.trainable_variables))
